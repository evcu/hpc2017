...
#pragma omp parallel
 ...
 for (i=0;i<N+2; i++){
  ...
  for (iter = 0; iter < max_iters && res/res0 > tol; iter++) {
    ...
    // if (iter == 0){
    //   for (i = 1; i <= N; i++){
    //      for (j = 1; j <= N; j++){  //red nodes
    //          unew[i][j]  = 0.25 * (hsq + unew[i-1][j] + unew[i][j-1] + u[i+1][j]+ u[i][j+1]);
    //     }
    //   }
    // }
    ...
    //write the new values to u, such that when they swapped unew has the most recent values 
    #pragma omp parallel for default(none) shared(N,u,unew,hsq) private(i,j)
      for (i = 1; i <= N; i++){
